...
traces = {'Line': 'line', 'Bar': 'bar', 'Scatter': 'sca','Image':'img'}
data = []
...
def register_subplots(app, fig, go):
    ...
    def update_subplots(n_clicks, rows, cols, v_space, h_space,
        ...
        if triggered_id == 'trace-table' and previous_data:
            deleted_rows = [row for row in previous_data if row not in current_data]
            for deleted_row in deleted_rows:
                new_fig.data = [trace for trace in new_fig.data if trace.name != deleted_row['ID']]

        for trace in current_data:
            trace_x = df[trace['XData']] if not df.empty else [1]
            trace_y = df[trace['YData']] if not df.empty else [1]
