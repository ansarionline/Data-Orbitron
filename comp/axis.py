...
def make_xaxis():
    return dbc.Form([
        html.H5('X Title',style={'margin':'5px'}),
        dbc.Container(id='x-title-div', children=[
            dbc.Input(id='x-title-text', value='X Axis', placeholder='X Title'),
            dbc.InputGroup([
                dbc.Select(id='x-title-font', placeholder='Font',
                        value='Arial',options=[{'label':l,'value':l} for l in fonts]),
                dbc.Input(id='x-title-size', type='number', 
                        min=5, step=1,value=12,
                        placeholder='Font Size'),
                dbc.Input(id='x-title-color', type='color', 
                        style={'height': '40px'},
                        value='#000000')
        ],style={'margin':'5px'})]),
        html.H5('X Tick',style={'margin':'5px'}),
        dbc.Container(id='x-tick-div', children=[
            dbc.InputGroup([
                dbc.Select(id='x-tick-font', placeholder='Font',
                        options=[{'label':l,'value':l} for l in fonts]),
                dbc.Input(id='x-tick-size', type='number', min=5,
                step=1, placeholder='Size',value=12),
                dbc.Input(id='x-tick-color', type='color', 
                style={'height': '40px'}, placeholder='Color',
                value='#000000'),
                dbc.Input(id='x-tick-angle', type='number', 
                min=0, max=180, step=1,value=0,
                placeholder='Angle')
            ]),
        ],style={'margin':'5px'}),
        html.H5('X Grid',style={'margin':'5px'}),
        dbc.Container(id='x-grid-div', children=[
            dbc.InputGroup([
                dbc.Checkbox('x-grid-show',value=False),
                dbc.Input(id='x-grid-color', type='color', value='#000000',
                style={'height': '40px'}, placeholder='Grid Color'),
                dbc.Input(id='x-grid-width', type='number',value=1,
                min=0, step=1, placeholder='Grid Width')
            ])
        ],style={'margin':'5px'}),
        html.H5('X Line',style={'margin':'5px'}),
        dbc.Container(id='x-line-div', children=[
            dbc.InputGroup([
                dbc.Input(id='x-line-color', type='color', value='#000000',
                style={'height': '40px'}, placeholder='Color'),
                dbc.Input(id='x-line-width', type='number',value=1,
                min=1, step=1, placeholder='Width')
            ])
        ],style={'margin':'5px'}),
    ],style={'margin':'5px'})
...
def make_yaxis():
    return dbc.Form([
        html.H5('Y Title',style={'margin':'5px'}),
        dbc.Container(id='y-title-div', children=[
            dbc.Input(id='y-title-text', value='Y Axis', placeholder='Y Title'),
            dbc.InputGroup([
                dbc.Select(id='y-title-font', placeholder='Font',
                        value='Arial',options=[{'label':l,'value':l} for l in fonts]),
                dbc.Input(id='y-title-size', type='number', 
                        min=5, step=1,value=12,
                        placeholder='Font Size'),
                dbc.Input(id='y-title-color', type='color', 
                        style={'height': '40px'},
                        value='#000000')
        ],style={'margin':'5px'})]),
        html.H5('Y Tick',style={'margin':'5px'}),
        dbc.Container(id='y-tick-div', children=[
            dbc.InputGroup([
                dbc.Select(id='y-tick-font', placeholder='Font',
                        options=[{'label':l,'value':l} for l in fonts]),
                dbc.Input(id='y-tick-size', type='number', min=5,
                step=1, placeholder='Size',value=12),
                dbc.Input(id='y-tick-color', type='color', 
                style={'height': '40px'}, placeholder='Color',
                value='#000000'),
                dbc.Input(id='y-tick-angle', type='number', 
                min=0, max=180, step=1,value=0,
                placeholder='Angle')
            ]),
        ],style={'margin':'5px'}),
        html.H5('Y Grid',style={'margin':'5px'}),
        dbc.Container(id='y-grid-div', children=[
            dbc.InputGroup([
                dbc.Checkbox('y-grid-show',value=False),
                dbc.Input(id='y-grid-color', type='color', value='#000000',
                style={'height': '40px'}, placeholder='Grid Color'),
                dbc.Input(id='y-grid-width', type='number',value=1,
                min=0, step=1, placeholder='Grid Width')
            ])
        ],style={'margin':'5px'}),
        html.H5('Y Line',style={'margin':'5px'}),
        dbc.Container(id='y-line-div', children=[
            dbc.InputGroup([
                dbc.Input(id='y-line-color', type='color', value='#000000',
                style={'height': '40px'}, placeholder='Color'),
                dbc.Input(id='y-line-width', type='number',value=1,
                min=1, step=1, placeholder='Width')
            ])
        ],style={'margin':'5px'}),
    ],style={'margin':'5px'})
...
def make_axis(app, fig):
    return dbc.Container(
        [   
            dbc.InputGroup([dbc.Label('Index',style={'margin':'5px'}),dbc.Select(id='axes-select')],
            style={'margin':'5px'}),
            html.H4('X Axis',style={'backgroundColor':'lightblue',
                                    'margin':'5px',
                                    'text-align':'center',
                                    'border-radius':'10px'}),
            dbc.Row(dbc.Col(make_xaxis()), className="mb-3"),
            html.H4('Y Axis',style={'backgroundColor':'lightblue',
                                    'margin':'5px',
                                    'text-align':'center',
                                    'border-radius':'10px'}),
...
def register_xaxis(app, fig):
    ...
    def default_xaxis_title(axes, layout):
...
def register_yaxis(app, fig):
    ...
    def default_yaxis_title(axes, layout):
        index = axes if axes is not None and int(axes) > 1 else ''
        layout = json.loads(layout)
